...
int Hi3518_test_EncGetStream(int nChannel, int nStreamType, void *stream, int *size, int fpH264File)
{
	HI_S32 s32Ret;
	HI_S32 s32VencFd = -1;
	HI_S32 VeChn = 0;
	...
	*size = 0;
	...
	{
		printf("select ret waitting time = %d\n", s32Ret);
		return -1;
	}
	else
	{
...
int videoEncFunc_v()
{
	...
	while (1)
	{
		...
		ret = Hi3518_test_EncGetStream(0, 0, stream, &size, fpH264File);
...
//    return pthread_create(&gs_VencPid, 0, SAMPLE_COMM_VENC_GetVencStreamProc, (HI_VOID*)&gs_stPara);
	return pthread_create(&gs_VencPid, 0, videoEncFunc_v, NULL);
}
