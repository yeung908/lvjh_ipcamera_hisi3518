...
/*
void gpio_write(unsigned char devaddress, unsigned char address, unsigned char data)
{
    unsigned char  tmp = 0;
    gpio_i2c_write(devaddress, address, data);
    tmp = gpio_i2c_read(devaddress, address);
    printk("write %x %x %x return %x\n", devaddress, address, data, tmp);
    if (!(devaddress == 0x72 && address == 0x1a))
    {
        tmp = gpio_i2c_read(devaddress, address);
    }
    printk("write %x %x %x return %x\n", devaddress, address, data, tmp);  
...
unsigned char gpio_read(unsigned char devaddress, unsigned char address)
{
    unsigned char tmp;
    tmp = gpio_i2c_read(devaddress, address);
   printk("read %x %x return %x\n", devaddress, address,tmp);
...
unsigned char _hi_i2c_read_byte(unsigned char devaddress, unsigned char address)
{
    /*
   unsigned char tmp;
   tmp = (HI_I2C_Read(devaddress, address, 1, 1) & 0xff); 
   printk("read %x %x return %x\n", devaddress, address,tmp);
...
void _hi_i2c_write_byte(unsigned char devaddress, unsigned char address, unsigned char data)
{
    //unsigned char tmp = 0 ;
...
#define  gpio_i2c_write I2CWriteByte
#endif
/*----------------------------------------------------------------------------
...
{
    TPI_TRACE_PRINT(("siiReadSegmentBlockEDID\n")); 
   return 1;
}
...
byte DoEdidRead (void)
{
	...
	if (g_edid.edidDataValid == FALSE)
	{
		...
		else
		{
            ...
            /*下边三行是为了解决Hi3518在以下条件成粉红色:
            1. pinmux+拨码到bt1120,cvbs输出->insmod sil9024.ko  ->BT1120输出
            */
         	g_edid.HDMI_Sink = FALSE;
		g_edid.YCbCr_4_4_4 = FALSE;
		g_edid.YCbCr_4_2_2 = FALSE;
...
byte SetEmbeddedSync (void)
{
	...
	WriteByteTPI(TPI_DE_CTRL, 0x30);
    	...
    	TPI_TRACE_PRINT(("ModeTblIndex=%x\n", ModeTblIndex));
      ...
      TPI_TRACE_PRINT(("EmbeddedSync:%x %x %x %x %x %x %x %x\n",
        B_Data[0], B_Data[1], B_Data[2], B_Data[3], B_Data[4], B_Data[5], B_Data[6], 
        B_Data[7], B_Data[8]));
...
void SetFormat (byte *Data)
{
	...
	//printk(KERN_INFO "sil9024 SetFormat OUTPUT_MODE_HDMI\n");
	...
	if (!IsHDMI_Sink()) 
	{
		ReadModifyWriteTPI(TPI_SYSTEM_CONTROL_DATA_REG, OUTPUT_MODE_MASK, OUTPUT_MODE_DVI);
	       TPI_DEBUG_PRINT(("sil9024 SetFormat OUTPUT_MODE_DVI....\n"));
	}
...
byte InitVideo (byte TclkSel)
{
	...
	B_Data[7] = (VModesTable[ModeTblIndex].Tag.Total.Lines >> 8) & 0xFF;
       ...
       TPI_TRACE_PRINT(("InitVideo:%x %x %x %x %x %x %x %x\n", B_Data[0], B_Data[1],
        B_Data[2], B_Data[3], B_Data[4], B_Data[5], B_Data[6], B_Data[7])); 
...
#endif
      TPI_TRACE_PRINT(("bYCbCr_4_4_4:%x,YCbCr_4_2_2:%x,B_Data %x %x\n",
      g_edid.YCbCr_4_4_4, g_edid.YCbCr_4_2_2, B_Data[0], B_Data[1]));
...
void OnDownstreamRxPoweredDown (void)
{
	...
	DisableTMDS();
       ...
       TPI_DEBUG_PRINT(("OnDownstreamRxPoweredDown OUTPUT_MODE_DVI....\n"));
}
...
void siHdmiTx_VideoSel (byte vmode)
{
	...
	siHdmiTx.ColorSpace 	    = YCBCR422_16BITS;//RGB;YCBCR444,YCBCR422_16BITS
	siHdmiTx.ColorDepth			= VMD_COLOR_DEPTH_8BIT;
...
// Globals: none
//------------------------------------------------------------------------------
static int sil902x_open(struct inode * inode, struct file * file);
static int sil902x_close(struct inode *inode ,struct file *file);
//static int sil902x_ioctl(struct inode *inode, struct file *file, unsigned int cmd, unsigned long arg);
static int sil902x_ioctl(struct file *file, unsigned int cmd, unsigned long arg);
...
static int sil902x_device_init(void);

static struct file_operations sil902x_fops = {
	.owner		= THIS_MODULE,
//	.ioctl		= sil902x_ioctl,
	.unlocked_ioctl		= sil902x_ioctl,	
	.open		= sil902x_open,
	.release	= sil902x_close
};
static struct miscdevice sil902x_dev = {
	MISC_DYNAMIC_MINOR,
	DEV_NAME,
	&sil902x_fops,
};
...
static int sil902x_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
    switch(cmd)
    {
        case HDMI_CMD_SETMODE:
        {
            if((arg>1)||(arg<12))
            {
                norm = arg;
                del_timer_sync(&sil902x_timer);

                if(sil902x_device_init() < 0)
                {
                    misc_deregister(&sil902x_dev);
                    DPRINTK(0, "sil902x device init fail,deregister it!");
                    return -1;
                }
            }
            else
...
#if 1	
	init_timer(&sil902x_timer);
	sil902x_timer.function = siHdmiTx_TPI_Poll;
	//sil902x_timer.data = (unsigned long) host;
	sil902x_timer.expires = jiffies + TIMER_DELAY;
	add_timer(&sil902x_timer);
#endif
    // add by z39183, drv cfg for embeded sync mode miss the following cfg.
    msleep(1000); // why? I have to add the time because the following I2C is invalid without it. 9024 need to be stable??

    #if 0
    gpio_i2c_write(0x72, 0x63, 0x40);
    printk("0x63 = 0x%x\n",gpio_i2c_read(0x72, 0x63));
    #else


 //   I2CWriteByte(0x72, 0x63, 0x40);
    printk("0x63 = 0x%x\n",I2CReadByte(0x72, 0x63));      
    I2CWriteByte(0x72, 0x64, 0x4C); 
...
{
	//HW Reset
	I2CWriteByte(0x72,0x82,0x25);
	I2CWriteByte(0x72,0x08,0x36);
	I2CWriteByte(0x72,0x7c,0x14);
	I2CWriteByte(0x72,0xc7,0x00);
	DelayMS(100);

	//9022 PLL Set:
	I2CWriteByte(0x72,0xBC,0x01);
	I2CWriteByte(0x72,0xBD,0x82);
	I2CWriteByte(0x72,0xBE,0x35);

	//9022A PLL Set
	I2CWriteByte(0x72,0x08,0x70);

	I2CWriteByte(0x72,0x00,0x8C);
	I2CWriteByte(0x72,0x01,0x0A);
	I2CWriteByte(0x72,0x02,0x70);
	I2CWriteByte(0x72,0x03,0x17);
	I2CWriteByte(0x72,0x04,0x5A);
	I2CWriteByte(0x72,0x05,0x03);
	I2CWriteByte(0x72,0x06,0x0D);
	I2CWriteByte(0x72,0x07,0x02);

	I2CWriteByte(0x72,0x63,0x00);	//bit6=1 for DE

	//Color Space
	I2CWriteByte(0x72,0x08,0x70);
	I2CWriteByte(0x72,0x09,0x00);
	I2CWriteByte(0x72,0x0A,0x00);

	I2CWriteByte(0x72,0x60,0x04);
	...
	//Power Up
	I2CWriteByte(0x72,0x1E,0x00);

	//Set AVI
	//I2CWriteByte(0x72,0x0C,14);
#if 1
	I2CWriteByte(0x72,0x09,0x02);	//  ---Y422 input

	I2CWriteByte(0x72,0x0A,0x00);	// ---RGB output

	I2CWriteByte(0x72,0x19,0x00);	// ---enable color convert, must do it here

	I2CWriteByte(0x72,0x60,0x80);	// ---embeded sync select

	I2CWriteByte(0x72,0x62,0x58);	// ---HBIT_2_HSYNC L

	I2CWriteByte(0x72,0x63,0x40);	// ---embede enable and HBIT_2_HSYNC M

	I2CWriteByte(0x72,0x64,0x4C);	// ---FIELD2_OFST L

	I2CWriteByte(0x72,0x65,0x04);	// ---FIELD2_OFST M

	I2CWriteByte(0x72,0x66,0x2C);	// ---HWIDTH L

	I2CWriteByte(0x72,0x67,0x00);	// ---HWIDTH M

	I2CWriteByte(0x72,0x68,0x02);	// ---VBIT_2_VSYNC

	I2CWriteByte(0x72,0x69,0x05);	// ---VWIDTH
#endif


	//Enable TMDS
	I2CWriteByte(0x72,0x1A,0x00);

	return;
	
}
static int __init sil902x_init(void)
{
	volatile unsigned int ret= 0;

	ret = misc_register(&sil902x_dev);
	if(ret)
	{
...
static void __exit sil902x_exit(void)
{
//    sil902x_clear_reg();
