#ifndef __AENC_MODULE_H_
#define __AENC_MODULE_H_

#define MAX_AENC_CHANNEL 4

typedef enum 
{
	// 编码参数 
    aenc_setup_enc_param,
    aenc_get_enc_param,
...
typedef struct
{
	...
	unsigned long nMode;		// 编码模式：0：立体声，1：MS立体声，2：双通道，3
...
typedef struct aencModuleInfo  *aencModuleInfo_t;

typedef int (*aencOpenFun_t)(int nChannel, int encType);
typedef int (*aencCloseFun_t)(int nChannel);
typedef int (*aencSetupFun_t)(int opt, void *param);
typedef int (*aencGetSetupFun_t)(int opt, void *param);
typedef int (*aencStartFun_t)(int nChannel);
typedef int (*aencStopFun_t)(int nChannel);
typedef int (*aencGetStreamFun_t)(int nChannel, void *stream, int *size);
typedef int (*aencReleaseStreamFun_t)(int nChannel);
typedef int (*aencGetAudioFun_t)(int nChannel, void *stream, int *size);
typedef int (*aencReleaseAudioFun_t)(int nChannel);

struct aencModuleInfo 
{
    aencOpenFun_t			open;
    aencCloseFun_t			close;
    aencSetupFun_t			setup;
    aencGetSetupFun_t			getSetup;
    aencStartFun_t				start;
    aencStopFun_t				stop;
    aencGetStreamFun_t		getStream;
    aencReleaseStreamFun_t		releaseStream;
	aencGetAudioFun_t			getAudio;
	aencReleaseAudioFun_t		releaseAudio;
