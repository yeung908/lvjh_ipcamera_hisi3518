...
::handleCmd_withinSession(char const* cmdName,
  ...
  ServerMediaSubsession* subsession;
 // printf("fullRequestStr = %s urlSuffix = %s urlPreSuffix = %s\n", fullRequestStr, urlSuffix, urlPreSuffix);
  if (urlPreSuffix[0] == '\0' && urlSuffix[0] == '*' && urlSuffix[1] == '\0') {
   ...
   // handleCmd_notFound(cseq);
	subsession = NULL;
	printf("fOurServerMediaSession->streamName() = %s\n", fOurServerMediaSession->streamName());
    //return;
  }
